...
export const useChatStore = defineStore('chat', {
      ...
      this.subscription = supabase
        ...
        .on(
          'postgres_changes',
          {
            ...
            table: 'messages',
            filter: areaId 
              ? `world_id=eq.${worldId},area_id=eq.${areaId}`
              : `world_id=eq.${worldId},area_id=is.null`
          },
          (payload) => {
    ...
    async fetchCharactersInArea(areaId) {
      ...
      if (!error && characters) {
        this.charactersInArea = characters.map(character => ({
          ...character,
          username: character.profiles?.username || `User_${character.user_id.slice(0, 8)}`,
  ...
  getters: {
    ...
    unreadCount: (state) => {
      return state.messages.filter(msg => !msg.read && msg.user_id !== useSupabaseUser()?.value?.id).length
    },
